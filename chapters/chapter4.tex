...
observed in data due to mismodelling effects arising from the matrix element generator, parton
...
make a direct measurement of $\mu_{\text{pre-tag}}$ using $Z \rightarrow b\bar{b}$ events
...
$H \rightarrow b\bar{b}$ and $H \rightarrow c\bar{c}$ searches, and correctly modelling
...
is required to correct for mismodelling of the top-jet mistag rate, which is the rate at
...
The normalisation factor $\mu_{t\bar{t}}$ absorbs the known data-MC discrepancy in the
$t\bar{t}$ $p_\text{T}$ spectrum, while $SF$ isolates the efficiency correction
introduced by the tagger itself. Since no floating normalisation is applied to the
...
probe-jet $p_\text{T}$ by simultaneously extracting the $t\bar{t}$ normalisation factor
...
the GN2Xv01 working points had not yet been finalised; a flat threshold of 1.76 on the
...
finalised recommendations from the JETMET group were not yet available. Also observed is a
...
Figure~\ref{fig:GN2X_Discriminant}. The normalisation factor $\mu_{t\bar{t}}$ and scale
factor $SF$ are extracted simultaneously from the SR and CR in each $p_\text{T}$ bin,
ensuring that any overall $t\bar{t}$ normalisation difference is absorbed by $\mu_{t\bar{t}}$
and does not bias the extracted $SF$.

\subsubsection{\texorpdfstring{$t\bar{t}$}{ttbar} Mistag Calibration Results}

The $t\bar{t}$ normalisation factor $\mu_{t\bar{t}}$ and scale factor $SF$ are extracted
per probe-jet $p_\text{T}$ bin from the simultaneous maximum likelihood fit to the SR and
CR mass distributions. These results include statistical uncertainties only; a finalised
result would additionally incorporate systematic uncertainties from sources including jet
energy and mass scale and resolution, $t\bar{t}$ modelling from initial- and final-state
radiation variations, and flavour tagging uncertainties.
...
\begin{figure}[h]
	...
	\end{subfigure}
	\caption{Extracted $t\bar{t}$ normalisation factor $\mu_{t\bar{t}}$ and scale factor
...
normalisation and $p_\text{T}$ modelling between the two configurations. Despite this
difference in normalisation, the scale factors extracted from MC16 and MC20 are in good
...
mismodelling of the top-quark mistag rate in either MC campaign.
