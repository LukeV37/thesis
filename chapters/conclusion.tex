...
This thesis presented three contributions to the ATLAS physics program, united by the application of machine learning to jet physics problems at the LHC and HL-LHC.
Together, they span the full lifecycle of a measurement: from calibrating the tools, to making discoveries, to preparing for the next generation of physics.
...
The common thread across these contributions is the use of modern graph and attention-based neural network architectures to extract physics information from variable-length, set-structured inputs --- tracks within a jet, particles within an event, or jet constituents within a boosted decay.
Each contribution pushed the state of the art in its domain while building toward the shared goal of precision Higgs and top quark physics at the LHC and HL-LHC.
